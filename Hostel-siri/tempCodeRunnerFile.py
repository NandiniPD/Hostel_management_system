...
def submit_complaint():
    if request.method == 'POST':
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO complaints (student_id, student_name, complaint) VALUES (%s, %s, %s)",
            (current_user.id, current_user.email, complaint_text)
        )
        conn.commit()
        cursor.close()
        conn.close()
...
def view_complaints():
    ...
    cursor.execute("SELECT id, student_name, complaint_text, status FROM complaints WHERE student_id IN (SELECT id FROM students)")
    complaints = cursor.fetchall()
